...
public class DynamicDialogRequestFactory {

    private DynamicDialogRequestFactory() {
    }

    public static DialogMatchApiInput fromStorage(Context context) {
        ...
        return new DialogMatchApiInput(
    ...
    private static String normalizeMobile(String value) {
        ...
        if (mobile.startsWith("Old")) mobile = mobile.substring(3);
        mobile = mobile.replaceAll("[^0-9]", "");
        if (mobile.length() == 10 && mobile.startsWith("9")) return "0" + mobile;
        return mobile;
    }
}
